...
export default function createGenerateClassName(options = {}) {
  ...
  return (rule, styleSheet) => {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (ruleCounter >= 1e10) {
        console.warn(
          ['blocks-ds: you might have a memory leak.', 'The ruleCounter is not supposed to grow that much.'].join('')
        );
      }
    }
    ...
    // Is a global static blx style?
    if (name && name.indexOf('blx') === 0 && !styleSheet.options.link && !disableGlobal) {
